		...
		<div class="container">
			...
			<div class="collapse navbar-collapse" id="home-navbar-collapse">
				...
				<ul class="nav navbar-nav">
					...
					<li><a href="${pageContext.request.contextPath}/userManage/findUsers.action">后台管理</a></li>
					</c:if>
				</ul>
...
<!-- 												style="color:red;font-size:20px;text-align:center;"></label> -->
									...
									<div class="row">
										<div class="col-md-7 col-xs-7">
											<div class="form-group">
												<label for="add_income_category" class="control-label">收入类型*</label>
											<select class="form-control" id="add_income_category"
												name="shouzhiCategory.szcid">
												<%
													List<ShouzhiCategory> incomeList = (List<ShouzhiCategory>) session.getAttribute("incomes");
													if (incomeList != null) {
														for (ShouzhiCategory c : incomeList) {
												%>
													<option value="<%= c.getSzcid() %>"><%= c.getSon_category() %></option>
												<%
														}
													}
												%>
											...
											<input type="hidden" name="user_id"
													value="<%= ((cn.zhku.jsj144.zk.financialManage.pojo.User) session.getAttribute("user")).getUid() %>">
											</div>
										</div>
							...
							<div class="col-md-12">
								...
								<form id="add_spend_form"
									...
									<div class="row">
										<div class="col-md-7 col-xs-7">
											<div class="form-group">
												<label for="add_spend_category" class="control-label">支出类型*</label>
											<select class="form-control" id="add_spend_category"
												name="shouzhiCategory.szcid">
												<%
													List<ShouzhiCategory> spendList = (List<ShouzhiCategory>) session.getAttribute("spends");
													if (spendList != null) {
														for (ShouzhiCategory c : spendList) {
												%>
													<option value="<%= c.getSzcid() %>"><%= c.getSon_category() %></option>
												<%
														}
