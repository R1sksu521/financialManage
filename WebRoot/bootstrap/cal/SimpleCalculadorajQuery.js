/*
 * jQuery SimpleCalculadora - Fixed version
 */
 jQuery.fn.extend({Calculadora: function(op) {
					...
					defaults = {
						...
						InputRetroceso:idInstancia+'Retroceso',
						EtiquetaRetroceso:'Del'
					}

                    var op = $.extend({}, defaults, op);
					Botones=op.Botones;
					Signos=op.Signos;
                    $(LaCalculadora).append('<input type="text" class="'+op.ClasetxtSalida+'" id="'+op.txtSalida+'" value="0" readonly>');
                    ...
                    $(LaCalculadora).append('<input type="button" id="'+op.InputRetroceso+'" class="btn3d btn btn-warning btn-lg btn-block" value="'+op.EtiquetaRetroceso+'">');
					...
					$('.'+NombreBotonesClase).click(function(){
						...
						if(vTecla=='='){
							try { salida.val(eval(salida.val())); } catch(e) { salida.val('Error'); }
						}
						else if(salida.val()=='0' || salida.val()==''){
							if(vTecla=='.'){salida.val('0.');}
							else if(Signos.indexOf(vTecla)>-1){salida.val('0'+vTecla);}
					...
					$("#"+op.InputRetroceso).click(function(){
						var s=$('#'+op.txtSalida).val();
						$('#'+op.txtSalida).val(s.length>1 ? s.slice(0,-1) : '0');
					});
	}
});
